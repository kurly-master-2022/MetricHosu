package org.metrichosu.mcollect.parser.seoulHumanTemperature
...
class SeoulHumanTemperature: ExternalSourceParser {
    ...
    override fun parseDataFromSource(mid: String): MetricValue {
        ...
        val requestResult = this.httpClient.send(getRequest, HttpResponse.BodyHandlers.ofString())
            .let {
                ObjectMapper().readValue(it.body(), SeoulHumanTemperatureDto::class.java)
            }

        return MetricValue(
            mid,
            Date.from(LocalDateTime.now().toInstant(ZoneOffset.ofHours(9))),
            requestResult.currentWeatherInfo.humanTemperature
        )
    }
}